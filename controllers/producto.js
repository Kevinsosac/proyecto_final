...
postAgregarproducto: async (req, res) => {
    try {
        const { codigo, nombre, descripcion, unidadMedida, precioUnitario, id_lote, iva, } = req.body;

        const productos = new Producto({codigo, nombre, descripcion, unidadMedida, precioUnitario, id_lote, iva,});
...
putproducto: async (req, res) => {
    try {
        const { id } = req.params
        const { nombre, codigo, descripcion, unidadMedida, precioUnitario, id_lote, iva,} = req.body
        const productos = await Producto.findByIdAndUpdate(id,{ nombre, codigo, descripcion, unidadMedida, precioUnitario, id_lote, iva,}, { new: true })
        await productos.save()
        res.json({ productos })
    } catch (error) {
        res.status(400).json({ error })
